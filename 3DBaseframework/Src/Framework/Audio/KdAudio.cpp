...
// ── mp3/m4a/aac/wma 等を 16bit PCM へデコードするための Media Foundation ──
#include <mfapi.h>
#include <mfidl.h>
...
namespace
{
	...
	// 圧縮音源(mp3等)か？（拡張子で判定）
	bool IsCompressedAudio(std::string_view name)
	{
		return EndsWithExtCI(name, ".mp3") ||
			EndsWithExtCI(name, ".m4a") ||
			EndsWithExtCI(name, ".aac") ||
			EndsWithExtCI(name, ".wma");
	}

	// ファイルパス(URL)から 16bit PCM へデコードする（mp3等）。
	//  outWfx に決定した PCM フォーマット、outPcm に全サンプルを詰めて返す。
	bool DecodeAudioToPcm(const wchar_t* path, std::vector<uint8_t>& outPcm, WAVEFORMATEX& outWfx)
	{
		...
		// 実際に決まった出力フォーマット（チャンネル数・サンプルレート）を取得
...
bool KdSoundEffect::Load(std::string_view fileName, const std::unique_ptr<DirectX::AudioEngine>& engine)
{
	if (engine.get() != nullptr)
	{
		try
		{
			...
			// mp3/m4a/aac/wma 等の圧縮音源は Media Foundation で PCM デコードして読み込む。
			// wav は従来どおり DirectXTK で直接読み込む（速い）。
			if (IsCompressedAudio(fileName))
			{
